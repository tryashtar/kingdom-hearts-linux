...
def main():
   ...
   if 'mods' not in settings and settings['installs'].get('kh1.5+2.5') is not None:
      ...
      if folder is not None:
         ...
         answer = yes_no()
         settings['mods']['refined'] = answer
         print()
         if answer:
            settings['mods']['openkh'] = True
         else:
            print('OpenKh mod manager: (y/n)')
            settings['mods']['openkh'] = yes_no()
            print()
         if settings['mods']['openkh']:
            print('OpenKh panacea mod loader: (y/n)')
            settings['mods']['panacea'] = answer == yes_no()
            print()
         print('LuaBackend script loader: (y/n)')
         settings['mods']['luabackend'] = answer == yes_no()
         print()
   ...
   if (wineprefix := settings.get('wineprefix')) is not None:
      ...
      if (saves := settings.get('saves')) is not None:
         if settings['mods'].get('refined'):
            if settings['installs'].get('kh1.5+2.5'):
               symlinks[os.path.join(user_folder, 'Documents/Kingdom Hearts/Configuration/1638')] = (os.path.join(saves, '1.5+2.5'), True)
               symlinks[os.path.join(user_folder, 'Documents/Kingdom Hearts/Save Data/1638')] = (os.path.join(saves, '1.5+2.5'), True)
         if settings['installs'].get('kh1.5+2.5'):
            symlinks[os.path.join(user_folder, 'Documents/KINGDOM HEARTS HD 1.5+2.5 ReMIX/Epic Games Store/1638')] = (os.path.join(saves, '1.5+2.5'), True)
         if settings['installs'].get('kh2.8'):
            symlinks[os.path.join(user_folder, 'Documents/KINGDOM HEARTS HD 2.8 Final Chapter Prologue/Epic Games Store/1638')] = (os.path.join(saves, '2.8'), True)
         if settings['installs'].get('kh3'):
            symlinks[os.path.join(user_folder, 'Documents/KINGDOM HEARTS III/Epic Games Store/1638')] = (os.path.join(saves, 'KH3'), True)
         if settings['installs'].get('mom'):
            symlinks[os.path.join(user_folder, 'Documents/KINGDOM HEARTS Melody of Memory/Epic Games Store')] = (os.path.join(saves, 'MoM'), True)
   ...
   if (kh_folder := settings['installs'].get('kh1.5+2.5')) is not None:
      ...
      if (mods_folder := settings['mods'].get('folder')) is not None:
         if settings['mods'].get('refined'):
            mod_folder = os.path.join(mods_folder, 'ReFined-KH2')
         ...
         if settings['mods'].get('openkh'):
            print('Downloading OpenKh...')
            mod_folder = os.path.join(mods_folder, 'OpenKh')
         ...
         if settings['mods'].get('luabackend'):
            print('LuaBackend...')
            mod_folder = os.path.join(mods_folder, 'LuaBackend')
